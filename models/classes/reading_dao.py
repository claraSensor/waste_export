    ...
    def get_items_to_export(self, fromdate, todate, devices, org_id):
        try:
            ...
            stmt = text(f'SELECT '
                        ...
                        f'WHERE lt.msg_type=\'1\' AND lt.anomalie = \'00\' AND data != \'2014-01-01\' AND  '
                        f'od.organization_id = {org_id} AND d.stato = {ReadingDao.STATUS_DEVICE_bool["active"]} '
                        f'AND lt.timestamp_rcv BETWEEN {int(fromdate.timestamp())} AND {int(todate.timestamp())} '
                        f'GROUP BY TO_CHAR(lt.data_ora_utc, \'DD/MM/YYYY\'), am.epc_str, am.tipo;')
